...
Encoder.prototype._startNc = function( cb ){
	// We're going to use nc to shove data to the different fragment
	// listeners.
	console.log( "Starting nc listener" );
...
Encoder.prototype._startFragmentLoop = function( cb ){
	...
	this._fragmentLoopHandler = ( data ) => {
		this._fragObj.handleData( Buffer.from( data ) );
	};
...
Encoder.prototype._startVideoFragment = function( cb ){
	async.waterfall( [ ( cb ) => {
		...
		const ffmpeg = spawn( "sh", [ "-c", "nc localhost 1337 | ffmpeg -f image2pipe -framerate 1 -i - -c:v libx264 -vf format=yuv420p -r 25 -f mp4 " + _path ] );
		...
		const handleData = ( data ) => {
			console.log( "Writing the data to nc.." );
